    ...
    <div class="col-md-8  mx-auto">
        ...
        <table class="table table-striped mt-5">
            ...
            @foreach($allnotices as $allnotice)
                <tr>
                <td class='col-8'>
                    <a href="{{ route('detail.adminNotice',['notice'=>$allnotice['id']]) }}">{{ $allnotice['title'] }}</a>
                </td>
                <td>
                    <a href="{{ route('edit.Notice',['notice'=>$allnotice['id']]) }}">
                        <button class="btn btn-primary btn-sm">編集</button>
                    </a>
                </td>
                <td>
                    <a href="{{ route('delete.Notice',['notice'=>$allnotice['id']]) }}"  class="btn btn-danger btn-sm" onclick="return confirm('本当に削除してもよろしいですか？')">削除</a>
                </td>
                </tr>
            @endforeach
        </table>
